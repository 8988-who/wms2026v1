 ...
 * @Author: YangZheng
 * @CreateTime: 2026-07-30 10:37
 * @Description: 离线站点PLC信号 → AGV补料/取空任务处理器
 * @Version: 1.0
 */
@Slf4j
@Component("offlinePlcHandler")
public class OfflinePlcHandler implements PlcSignalHandler {
    ...
    public void onSignal(String plcId, String plcAddress, String mesAddress, String exceptionAddress, short newVal) {
        switch (newVal) {
            case 0:
                noStatus(plcId, plcAddress, mesAddress, exceptionAddress);
                break;
            case 10: // 送空货架
                // 创建任务并获取任务单号
    ...
    public void noStatus(String plcId, String address, String mesAddress, String exceptionAddress) {
        log.info("PLC当前无状态,IP:{}", address);
        plcAdapterService.writeUInt16(plcId, mesAddress, 0);
        plcAdapterService.writeUInt16(plcId, exceptionAddress, 0);
    }
     ...
     * TaskTyp = PLC10
     *
     * @Description: PLC请求上位机送空货架（找空储位作为起点送到工位）
    ...
    public String stockUp(String plcId, String plcAddress, String mesAddress, String exceptionAddress, int triggerValue) {
        ...
        TWmsPlcReplenishmentConfig startArea = pcs.findByAddressAndTriggerValue(plcId, plcAddress, triggerValue);
        if (startArea == null) {
        ...
        log.info("查找起点(空储位)");
        TWmsStorage emptyEnd = findEmptyEnd(startArea.getAreaId());
        if (emptyEnd == null) {
            log.error("未找到空储位: areaId={}", startArea.getAreaId());
            plcAdapterService.writeUInt16(plcId, exceptionAddress, 40);
            return null;
        }
        TWmsItemInfo start = new TWmsItemInfo();
        start.setItemCarNo(emptyEnd.getItemCarNo());
        start.setAgvCode(emptyEnd.getAgvStorageCode());
     ...
     * TaskTyp = PLC20
     *
     * @Description: PLC请求上位机取走满货架（工位作为起点，满料储位作为终点）
     * @Param: [plcId, address, triggerValue]
     * @return: void
     */
    public String takeEmptyShelf(String plcId, String plcAddress, String mesAddress, String exceptionAddress, int triggerValue) {
        ...
        TWmsStorage end = plcService.findEnd(config.getOnlineType(), config.getItemModelCode());
        if (end == null) {
            log.error("未找到满料储位: onlineType={}, itemModelCode={}", config.getOnlineType(), config.getItemModelCode());
            plcAdapterService.writeUInt16(plcId, exceptionAddress, 40);
            return null;
        }
    ...
    public void cancelAgvTask(String plcId, String plcAddress) {
        // 拿配置信息
        TWmsPlcLocationConfig locationConfig = plcs.findByAddress(plcId, plcAddress);
        if (locationConfig == null || locationConfig.getAgvTaskCode() == null) {
            log.error("未找到工位任务配置: plcId={}, address={}", plcId, plcAddress);
